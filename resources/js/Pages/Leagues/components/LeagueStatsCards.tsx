...
import { Trophy, Calendar, Route, Play, Flag } from 'lucide-react';
...
export function LeagueStatsCards({ league, next_stage, user_position, isOngoing, isFinished, onCountdownExpired, onFinishedClick }: LeagueStatsCardsProps) {
    return (
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
            ...
            {isFinished ? (
                <button type="button" onClick={onFinishedClick} className="block text-left w-full">
                    <Card className="cursor-pointer transition-all h-full border-gray-300 dark:border-gray-600 hover:bg-muted/50">
                        <CardContent className="flex flex-col items-center justify-center px-3 py-5 sm:px-4 sm:py-6">
                            <Flag className="mb-1 h-4 w-4 text-gray-500" />
                            <div className="text-lg font-bold sm:text-xl">Finalizada</div>
                            <p className="text-[11px] text-muted-foreground">Ver resultados</p>
                        </CardContent>
                    </Card>
                </button>
            ) : (
